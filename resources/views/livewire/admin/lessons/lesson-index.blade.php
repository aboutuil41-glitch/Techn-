<div class="space-y-6 font-[Jost]">
    ...
    <div class="overflow-hidden rounded-[1.8rem] border border-[#E8D7DB] bg-[#FFFCFA] shadow-[0_16px_42px_rgba(192,57,43,0.07)]">
        <div class="flex flex-col gap-5 border-b border-[#E8D7DB] bg-[#FDF8F6] px-6 py-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <p class="text-[11px] uppercase tracking-[0.18em] text-[#A58A99]">Content</p>
                <h3 class="mt-3 font-[Cormorant_Garamond] text-3xl font-medium tracking-tight text-[#1C1020]">Lessons</h3>
                <p class="mt-2 text-sm text-[#8A7090]">Handle lesson content, module assignment, and media links.</p>
            </div>
                        ...
                        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                            <div class="max-w-3xl">
                                <div class="flex flex-wrap items-center gap-3">
                                    <h4 class="font-[Cormorant_Garamond] text-3xl font-medium text-[#1C1020]">{{ $lesson->title }}</h4>
                                ...
                                <p class="mt-3 text-sm text-[#8A7090]">
                                    Module: <span class="font-medium text-[#4A3050]">{{ $lesson->module->title ?? '—' }}</span>
                                </p>

                                <p class="mt-2 text-xs uppercase tracking-[0.16em] text-[#A58A99]">
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-[#1C1020]/45 p-4">
            <div class="w-full max-w-3xl overflow-hidden rounded-[1.8rem] border border-[#E8D7DB] bg-[#FFFCFA] shadow-[0_24px_60px_rgba(28,16,32,0.28)]">
                ...
                <div class="p-6">
                    <livewire:admin.lessons.lesson-form
                        :lessonId="$editingId"
                        :key="'lesson-form-' . ($editingId ?? 'new')" />
                </div>
            </div>
        </div>
    @endif
